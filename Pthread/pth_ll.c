...
void* Thread_work(void* rank) {
   ...
   for (i = 0; i < ops_per_thread; i++) {
      ...
      if (which_op < search_percent) {
		 ...
		 read_count ++;
		 if(read_count == 1)pthread_mutex_lock(&wrt);
		 //pthread_cond_wait(&rd,&wrt);
		 ...
		 pthread_mutex_lock(&mutex);
		 read_count --;
		 if(read_count == 0)pthread_mutex_unlock(&wrt);
		 //pthread_cond_signal(&rd); //pthread_mutex_unlock(&wrt);
		 pthread_mutex_unlock(&mutex);
